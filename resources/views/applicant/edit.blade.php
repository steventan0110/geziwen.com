        ...
        <form action="{{ route('agency.applicants.update', ['agency' => $agency->id, 'applicant' => $applicant->id]) }}" method="post">
            ...
            <div class="bg-white box-shadow rounded p-3">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control" id="name" name="applicant[surname]" value="{{ $applicant->surname }}">
                </div>
                <div class="form-group mb-3">
                    ...
                    <div class="input-group mb-3">
                        <select type="email" class="form-control" id="email" name="applicant[plan]" >
                            @foreach($agency->plans as $plan)
                                <option {{ $plan->id == $applicant->plan->id ? 'selected' : '' }} value="{{ $plan->id }}">{{ $plan->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group mb-3">
                    ...
                    <textarea class="form-control" name="applicant[introduction]" id="introduction">{{ $applicant->introduction }}</textarea>
                </div>
                <div class="form-group mb-3">
